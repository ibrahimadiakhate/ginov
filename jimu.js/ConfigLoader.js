...
function (declare, lang, array, html, dojoConfig, cookie,
  ...
  arcgisUtils, ArcGISDynamicMapServiceLayer, FeatureLayer) {
    ...
    ********************************************************/
    ...
    /*var urlWebmap = this.urlParams.itemid || this.urlParams.webmap;
     if(urlWebmap && appConfig.map.itemId !== urlWebmap){
      ...
      if(this.urlParams.itemid && appConfig.map.itemId !== this.urlParams.itemid){
        if(appConfig.map.mapOptions){
          jimuUtils.deleteMapOptions(appConfig.map.mapOptions);
        }
        appConfig.map.itemId = this.urlParams.itemid;
      } else if (this.urlParams.webmap) {
        appConfig.map.webmap = this.urlParams.webmap;
        if(appConfig.map.mapOptions){
          jimuUtils.deleteMapOptions(appConfig.map.mapOptions);
        }
      }
    ...
    initLayers: function(map) {
      // Ajout via l'itemID du Portal
      if(this.urlParams.layers) {
        var portal = portalUtils.getPortal(portalUrl);

        var ids = [];
        if (this.urlParams.layersId) {
          ids = this.urlParams.layersId.split(",");
        }

        array.forEach(this.urlParams.layers.split(","), function(serviceId){
          portal.getItemById(serviceId).then(function(layer) {
            if (layer.type === "Map Service") {
              var layer = new ArcGISDynamicMapServiceLayer(layer.url, {
                  id: serviceId
              });
              map.addLayer(layer);
            } else if (layer.type === "Feature Service") {
              var layer = new FeatureLayer(layer.url + "/" + ids.shift(), {
                id: serviceId
              });
              map.addLayer(layer);
            }
      ...
      // Ajout via l'URL du services (modif magellium)
      if (this.urlParams.services) {
		var srvslayerIndex = null;
		if(this.urlParams.layerIndex){
			console.log("index de couche defini pour le service : " + this.urlParams["layerIndex"]);
			
			var tmp_array = new Array();
			var srvslayerIndex_arr = this.urlParams.layerIndex.split(",");
			for (i = 0; i < srvslayerIndex_arr.length; i++) {
				var srvslayerIndex_c = srvslayerIndex_arr[i];
				srvslayerIndex_c_arr = srvslayerIndex_c.split("_");
				if(srvslayerIndex_c_arr.length == 2){
					var srvslayerIndex_serviceNum = srvslayerIndex_c_arr[0];
					var srvslayerIndex_layerNum = parseInt(srvslayerIndex_c_arr[1]);
					if(tmp_array[srvslayerIndex_serviceNum]){
						tmp_array[srvslayerIndex_serviceNum].push([srvslayerIndex_layerNum]);
					}else{
						tmp_array[srvslayerIndex_serviceNum] = [srvslayerIndex_layerNum];
					}
				}
				
			}
			
			srvslayerIndex = tmp_array;
		}
		
		var nbrServiceUri = 0;
        array.forEach(this.urlParams.services.split(","), function(serviceURL){
		      var layer = null;
		      var layerAfterAdd = null;
		  
          if (serviceURL.indexOf("MapServer") > 0) {
            layer = new ArcGISDynamicMapServiceLayer(serviceURL);
            layerAfterAdd = map.addLayer(layer);
          } else if (serviceURL.indexOf("FeatureServer") > 0) {
            layer = new FeatureLayer(serviceURL);
            map.addLayer(layer);
          }
		  
		  // on verifie si des options de couche sont defini pour le service courant
		  if(srvslayerIndex != null && srvslayerIndex[nbrServiceUri + ""]){
			  
			// on prepare les information pour la layerlist
			var layerAfterAdd_id = layerAfterAdd.id;
			
			magLayersToShowInfos = {};
			magLayersToShowInfos[layerAfterAdd_id] = srvslayerIndex[nbrServiceUri + ""]
			console.log(magLayersToShowInfos);

			var visibleLayerIds = srvslayerIndex[nbrServiceUri + ""];
