  ...
  var PrintDijit = declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
    ...
    _handlePrintInfo: function(rData) {
      if (!rData.isGPPrint) {
        ...
        var tab_names = {"A4 Landscape" : "A4 Paysage", "A3 Landscape": "A3 Paysage", "MAP_ONLY" : "Emprise carte", "Letter ANSI A Landscape" : "Lettre ANSI A Paysage", "Tabloid ANSI B Landscape":"Tabloid ANSI B Paysage"};
        var layoutItems = array.map(Layout_Template[0].choiceList, function(item) {
          var label = (tab_names[item] != undefined) ? tab_names[item] : item;
          return {
            label: label,
            value: item
          };
