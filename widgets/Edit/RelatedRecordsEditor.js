  ...
  function (declare, lang, array, html, on, Deferred, domQuery, editUtils,
    ...
    var Clazz = declare([_WidgetBase, _TemplatedMixin], {
      ...
      _addNewRelatedRecord: function (operationData) {
		...
		//probleme liaison plantation - travail
		if(newRelatedRecordPara.attributes.id_pat == null){
			newRelatedRecordPara.attributes.id_pat = operationData.feature.attributes.globalid;
		}
		console.log(newRelatedRecordPara);
      ...
      _updateRelatedRecord: function (operationData, attributeChangeEvt) {
		...
		//bha modif
		//pour les travaux, il faut enlever les categories, activites, materiels et lac mat
		//anomalie il faut enlever operateur
		//console.log(operationData.destJimuLayerInfo.title);
		if(operationData.destJimuLayerInfo.title.includes("rav")){
			delete relatedFeature.attributes.categories;
			delete relatedFeature.attributes.activites;
			delete relatedFeature.attributes.materiels;
			delete relatedFeature.attributes.lac_mat;
			delete relatedFeature.attributes.prescripteur;
			delete relatedFeature.attributes.groupe;
			delete relatedFeature.attributes.operateur;
			
			
			delete relatedFeature.attributes.date_fin_prev;
			delete relatedFeature.attributes.date_fin_effe;
		}
		if(operationData.destJimuLayerInfo.title.includes("nom")){
			delete relatedFeature.attributes.operateur;
		}
	  ...
	  hasRelatedFeatures: function(id, url, btnD){
		//https://sig.seinegrandslacs.fr/arcgis/rest/services/Ginov/ginov_creation/FeatureServer/1/queryRelatedRecords?f=json&definitionExpression=&relationshipId=2&returnGeometry=false&objectIds=1601&outFields=*&token=iJBRyCJqBYVtNcrA0zu4f7mic7ZYR4JhUhkT2xU5nc4DaKctpMnsEGbBC4YrD3Mdvk4bG1cOSgS5Ou_E4KK2J7nGuVV73v94-zqvOrFtuPJNxQaTzn6BP3_nQbvEPwYGXQhSVxdbv8l8WM_MsGlS2o-AtVwzjb8FVh8MwgjD1Edgtew01VrHgdpeCvC1s7lB7wqmnarPKGAwgo77SQj3YlTXqH3pUOn8bC-LzfH3waQ.  
		  if(!url.includes("ginov_patrimoine")){
			  var scope2 = this;
        ...
        this._getRelatedTableInfoArray(operationData.oriJimuLayerInfo)
          .then(lang.hitch(this, function (layerInfoArray) {
            ...
            for (i = 0; i < atiAttributesListe.length; i++) {
              if (atiAttributesListe[i].textContent == "id_anomalie"){// || atiAttributesListe[i].textContent == "id_pat") {
                atiAttributesListe[i].style.display = "none";
              }
			  //partie pour plantation
			  /*if(atiAttributesListe[i].textContent == "id_pat"){
				  console.log(operationData.feature.attributes.globalid);
				  console.log(atiAttributesListe[i]);
				  atiAttributesListe[i].childNodes[1].childNodes[0].childNodes[1].childNodes[0].value = operationData.feature.attributes.globalid;
			  }*/
            }
      ...
      _setOperationTitle: function (title) {
		  //ibrahima => Mettre la premiere lettre en majuscule
        html.setAttr(this.operationTitle, 'innerHTML', title.charAt(0).toUpperCase() + title.slice(1));
      ...
      */
      ...
      printFrame: function (ext, tab, url, tab2, basemap) {
        ...
        printWindow = window.open(this.portalUrl + "/ginov/print.html?" +
          ...
          "&typegeom=" + tab2[0] + "&geom=" + tab2[1] + "&spatialref=" + tab2[2]+"&basemap="+basemap+"&token="+this.token);
      ...
      modifyFiche: function (attributeInspector, relatedLayerName) {
        if (relatedLayerName.includes("rav")) {
          ...
          for (i = 0; i < len; i++) {
            ...
            if (i != 6 && i != 10 && i != 8 && i != 14 && i != 15 && i != 16 && i != 0 && i != 11 && i != 12 && i != 17) {//les champs autorisés : statut, date fin effective, commentaire, durée
              if (i == 3 || i == 5 || i == 6 || i == 1 || i == 13) {//le cas d'une liste deroulante
            ...
            if (valueS != "Assigné" && valueS != "En cours") {
              if (i == 6 || i == 14 || i == 15 || i == 16 || i == 0 || i == 11 || i == 17) {
                if (i == 6 || i == 14 || i == 0 || i == 17) {
                  //console.log(i);
                  var id = elementi.childNodes[1].childNodes[0].childNodes[2].childNodes[0].id;
                }
                if (i == 15 || i == 16) {
      ...
      addPrintBtn: function (attributeInspector, operationData, relatedLayerName) {
        ...
        if (relatedLayerName.includes("rav")) {
          ...
          btn.onclick = function () {
            ...
            date_fin_prev = elementi.childNodes[13].childNodes[1].childNodes[0].childNodes[2].childNodes[0].value;
            date_fin_eff = elementi.childNodes[14].childNodes[1].childNodes[0].childNodes[2].childNodes[0].value;
            //commentaire = elementi.childNodes[15].childNodes[1].childNodes[0].childNodes[1].childNodes[0].value; 
			//ibrahima modif
			commentaire = elementi.childNodes[15].childNodes[1].childNodes[0].value;
            duree = elementi.childNodes[16].childNodes[1].childNodes[0].childNodes[1].childNodes[0].value;
      ...
      listeMateriels: function (attributeInspector, relatedLayerName) {
        ...
        if (relatedLayerName.includes("rav")) {
          ...
          var exist = elemAtt[12].childNodes[1].innerHTML;
          ...
          elemAtt[12].childNodes[1].innerHTML = exist;
          elemAtt[12].childNodes[1].childNodes[0].style.display = "none";
          ...
          var id = elemAtt[11].childNodes[1].childNodes[0].id;
          elemAtt[11].childNodes[1].childNodes[0].innerHTML = "";
          $("#" + id).append(select);
          ...
          */
			  if(scope.creationTrav == 1){
				  elemAtt[13].childNodes[1].childNodes[0].childNodes[2].childNodes[0].value = "";
				  elemAtt[14].childNodes[1].childNodes[0].childNodes[2].childNodes[0].value = "";
			  ...
			  if(elemAtt[13].childNodes[1].childNodes[0].childNodes[2].childNodes[1].value=="1970-01-01"){
				  elemAtt[13].childNodes[1].childNodes[0].childNodes[2].childNodes[0].value = "";
			  }
			  if(elemAtt[14].childNodes[1].childNodes[0].childNodes[2].childNodes[1].value=="1970-01-01"){
				  elemAtt[14].childNodes[1].childNodes[0].childNodes[2].childNodes[0].value = "";
			  }
	...
	//recuperer fullname
      ...
      disabledAttr: function (attributeInspector, relatedLayerName) {
        ...
        if (relatedLayerName.includes("rav")) {
          ...
          for (i = 0; i < len; i++) {
            ...
            if (i != 8 && i != 16) {
              if (i == 3 || i == 5 || i == 6 || i == 10 || i == 1 || i == 13 || i == 14 || i == 0) {//le cas d'une liste deroulante
                var id = elementi.childNodes[1].childNodes[0].childNodes[2].childNodes[0].id;
              }
              else if (i == 11) {
                var id = "selectLacMat";
      ...
      valueCateg: function () {
        var tabCateg = [{
		  ...
		  "Contrôle": [
            "Ajout de dispositif d'auscultation",
            "Prise de mesures"
          ],
		  "Dégrillage": [
			"Dégrillage manuel",
